...
========================================================================== */
  ...
  // Opciones de filtrado por categoria, tipo, plataforma y rango de precio.   
  $where = isset($_SESSION['filtro_where']) ? $_SESSION['filtro_where'] : '';

  if (isset($_POST['categoria']) && !empty($_POST['categoria'])
      || isset($_POST['tipo']) && !empty($_POST['tipo'])
      || isset($_POST['plataforma']) && !empty($_POST['plataforma'])
      || isset($_POST['rango']) && !empty($_POST['rango'])) {

      unset($_SESSION['filtro_where']);
      $where = ' WHERE ';
      $interruptor = true;
      ...
      $filters = [
          ...
          'tipo' => 'pr.productType',
          'plataforma' => 'pr.platformID',
          'rango' => 'pr.price * (1 - pr.discount / 100)',
      ];
      ...
      foreach ($filters as $key => $value) {
          if (isset($_POST[$key]) && !empty($_POST[$key])) {
              ...
              $clause = ($interruptor ? '' : ' OR ') . $value;
              if ($key === 'rango') {
                  $clause .= getRangeClause($selectedValues[0]);
              } else {
                  $clause .= " IN ('" . implode("', '", $selectedValues) . "')";
              }
              $where .= $clause;
              $interruptor = false;
          }
      ...
      $_SESSION['filtro_where'] = $where;

  } elseif (isset($_GET['page'])) {
      $where = isset($_SESSION['filtro_where']) ? $_SESSION['filtro_where'] : '';
  }

  // Función que para los rangos de precios del Filtro.
  function getRangeClause($precioMin) {
      if ($precioMin == 0) {
  ...
  // Recogemos la variable de la URL:
  $parametro = $_GET['p'];

  // Consulta para mostrar la plataforma en el breadcrumb y el título.
  if($parametro == 0){
    $plataforma = "Todas las plataformas";
  } elseif($parametro > 0 && $parametro <= 10) {
    $sqlPlataforma = "SELECT * FROM platform WHERE platformID = $parametro";
    $resultPlataforma = $mysqli->query($sqlPlataforma);
    while($filaPlataforma = $resultPlataforma->fetch_object()) {
        $plataforma = $filaPlataforma->platformName;
    }
  } else {
    $plataforma = "Promociones";
  }
  
  // Consulta para mostrar el listado de artículos.
  if($parametro == 0){ // Todos
    $sql = "SELECT pr.*, pl.platformName AS plataforma, pl.platformLogo AS logotipo, AVG(ra.value) AS valorMedio
            FROM product pr
            INNER JOIN platform pl ON pl.platformID = pr.platformID
            LEFT JOIN rating ra ON ra.productID = pr.productID
            GROUP BY pr.productID";
  } elseif($parametro == 11) { // Promociones
    $sql = "SELECT pr.*, pl.platformName AS plataforma, pl.platformLogo AS logotipo, AVG(ra.value) AS valorMedio
            FROM product pr
            INNER JOIN platform pl ON pl.platformID = pr.platformID
            LEFT JOIN rating ra ON ra.productID = pr.productID
  ...
  } elseif($parametro == 12) { // Accesorios
    $sql = 'SELECT pr.*, pl.platformName AS plataforma, pl.platformLogo AS logotipo, AVG(ra.value) AS valorMedio
            FROM product pr
            INNER JOIN platform pl ON pl.platformID = pr.platformID
            LEFT JOIN rating ra ON ra.productID = pr.productID
            WHERE pr.productType = "Accesorio"
            GROUP BY pr.productID';
  } elseif($parametro == 13) { // Filtros
    $sql = "SELECT pr.*, pl.platformName AS plataforma, pl.platformLogo AS logotipo, AVG(ra.value) AS valorMedio
            FROM product pr
            INNER JOIN platform pl ON pl.platformID = pr.platformID
            LEFT JOIN rating ra ON ra.productID = pr.productID " . $where . " GROUP BY pr.productID ORDER BY " . $order . "";
  } else  { // Juegos
    $sql = "SELECT pr.*, pl.platformName AS plataforma, pl.platformLogo AS logotipo, AVG(ra.value) AS valorMedio
            FROM product pr
            INNER JOIN platform pl ON pl.platformID = pr.platformID
            LEFT JOIN rating ra ON ra.productID = pr.productID
            WHERE pr.platformID = $parametro
            GROUP BY pr.productID";
  }
                ...
                <?php
                  if(isset($_GET['buscar'])) {
                    ...
                    if($parametro > 0 && $parametro <= 10){
                      echo '<li class="breadcrumb-item active" aria-current="page"><a href="products.php?p=0" target="_self" title="Enlace a la sección de juegos">Tienda</a></li>';
                      echo '<li class="breadcrumb-item" aria-current="page">Juegos de ' .$plataforma . '</li>';
                    } elseif($parametro == 11) {
                      echo '<li class="breadcrumb-item active" aria-current="page"><a href="products.php?p=0" target="_self" title="Enlace a la sección de juegos">Tienda</a></li>';
                      echo '<li class="breadcrumb-item" aria-current="page">Promociones</li>';
                    } elseif($parametro == 12) {
                      echo '<li class="breadcrumb-item active" aria-current="page"><a href="products.php?p=0" target="_self" title="Enlace a la sección de juegos">Tienda</a></li>';
                      echo '<li class="breadcrumb-item" aria-current="page">Accesorios</li>';
                    } elseif($parametro == 13) {
                      echo '<li class="breadcrumb-item active" aria-current="page"><a href="products.php?p=0" target="_self" title="Enlace a la sección de juegos">Tienda</a></li>';
                      echo '<li class="breadcrumb-item " aria-current="page">Productos filtrados</li>';
                    } elseif($parametro == 0) {
                      echo '<li class="breadcrumb-item active" aria-current="page"><a href="products.php?p=0" target="_self" title="Enlace a la sección de juegos">Tienda</a></li>';
                      echo '<li class="breadcrumb-item " aria-current="page">Juegos de ' .$plataforma . '</li>';
                    } 
            ...
            <?php
              if(isset($_GET['buscar'])) {
                echo '<h1 class="text-white">Resultados encontrados: ' . $num_total_rows . '</h1>';
                  ...
                  if($parametro > 0 && $parametro <= 10){
                    echo '<h1 class="text-white">Comprar productos de ' . $plataforma . '</h1>';
                  } elseif($parametro == 11) {
                    echo '<h1 class="text-white">' . $plataforma . '</h1>';
                  } elseif($parametro == 12) {
                    echo '<h1 class="text-white">Accesorios</h1>';
                  } elseif($parametro == 0) {
                    echo '<h1 class="text-white">Comprar productos de ' . $plataforma . '</h1>';
                  } 
              }
              ...
              <div class="filtrado col-11 d-flex justify-content-end align-items-center gap-3 gap-lg-2">
                <div class="btn-group mt-3 mt-lg-1">
                  ...
                  <ul class="dropdown-menu">
                    <li><a class="dropdown-item" href="products.php?order=1&p=<?= $parametro ?>">Precio: menor a mayor</a></li>
                    <li><a class="dropdown-item" href="products.php?order=2&p=<?= $parametro ?>">Precio: mayor a menor</a></li>
                    <li><a class="dropdown-item" href="products.php?order=3&p=<?= $parametro ?>">Valoraciones</a></li>
                    <li><a class="dropdown-item" href="products.php?order=4&p=<?= $parametro ?>">Últimos lanzamientos</a></li>
                  </ul>
                </div>
              ...
              <div class="offcanvas offcanvas-start" tabindex="-1" id="canvas-filtrado" aria-labelledby="offcanvascanvas-filtrado">
                ...
                <div class="offcanvas-body d-flex justify-content-center">
                  <form action="products.php?p=13&order=3" method="POST">
                      ...
                      <button id="aplicar" type="submit" class="btn btn-outline-dark btn-sm mt-2 mb-5 mb-lg-1 w-100">Aplicar cambios</button>
                  </form>
                </div>
              </div>
        </div>
            ...
            if ($num_total_rows > 0) {
              ...
              if($parametro == 0){
                ...
                $sql = 'SELECT pr.*, pl.platformName AS plataforma, pl.platformLogo AS logotipo, AVG(ra.value) AS valorMedio
                        FROM product pr
                        INNER JOIN platform pl ON pl.platformID = pr.platformID
                        LEFT JOIN rating ra ON ra.productID = pr.productID ' .$where. ' GROUP BY pr.productID ORDER BY ' . $order . ' LIMIT '.$start.', '.NUM_ITEMS_BY_PAGE.'';
              } else  {
                $sql = "SELECT pr.*, pl.platformName AS plataforma, pl.platformLogo AS logotipo, AVG(ra.value) AS valorMedio
                        FROM product pr
                        INNER JOIN platform pl ON pl.platformID = pr.platformID
              ...
              if ($total_pages > 1) {
                  if ($page != 1) {
                      ...
                      echo '<li class="page-item"><a class="page-link" href="products.php?p='.$parametro.'&order='.$ordenador.'&page='.($page-1).'"><span aria-hidden="true">&laquo;</span></a></li>';
                  }
                  ...
                  for ($i = 1; $i <= $total_pages; $i++) {
                      if ($page == $i) {
                          ...
                          echo '<li class="page-item"><a class="page-link" href="products.php?p='.$parametro.'&order='.$ordenador.'&page='.$i.'">'.$i.'</a></li>';
                      }
                  }

                  if ($page != $total_pages) {
                      // Aquí incluimos el parámetro de orden al avanzar una página
                      echo '<li class="page-item"><a class="page-link" href="products.php?p='.$parametro.'&order='.$ordenador.'&page='.($page+1).'"><span aria-hidden="true">&raquo;</span></a></li>';
                  }
              }
              echo '</ul>';
              echo '</nav>';
